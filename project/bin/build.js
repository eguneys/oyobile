...
module.exports.watch = function watch(f, settings, platform, configName) {
  var defer = Q.defer();
  ...
  gulp.add('watch-scripts', function() {
    return buildFile.watchScripts(srcFolder, assetsDest, mode, function(p) {
  ...
  gulp.add('launch-watch', function() {
    return buildFile.watchLaunch();
  });

  gulp.start('html', 'styl', 'watch-scripts', 'launch-watch', function(err) {
    if (err) defer.reject(err);
    else defer.resolve();
  });
...
module.exports.watchOLD = function watchOLD(f, settings, platform, config) {
